...
calls/results, cited assistant content, token usage, failures, and trace
context. Farfield credentials and the Anthropic API key are never included in
...
Each invocation prints its conversation and trace IDs and appends a compact
index to `test-traces.jsonl`. The authoritative trace remains in the configured
object store. Query the latest retained trace without copying any data out of
Farfield:
